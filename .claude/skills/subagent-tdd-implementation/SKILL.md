...
**モデル選択（必須）**: 委譲するサブエージェントには、**メインエージェントより必ず下位のモデルを指定する**（`Agent` ツールの `model` 引数）。モデルの上下関係は `fable` > `opus` > `sonnet` > `haiku`。

- メインが `fable` → サブエージェントは `opus` / `sonnet` / `haiku`
...
- 対象ステップの要求と完了基準
- **t-wada推奨TDD（Red-Green-Refactor）で進めること** → `tdd` スキル準拠。**テストを1件追加 → 実行してRedを確認 → 最小実装でGreen、を1件ずつ繰り返すこと。テストと実装を同時に書かないこと**を明記する
- **各テストのRed時の失敗出力（抜粋）を報告に含めること**を必須として明記する
...
- **コミット末尾のトレーラー（`Co-Authored-By` / `Claude-Session`）はメインが委譲プロンプトに明記し、サブエージェントはそれをそのまま使う**こと（サブエージェント側のセッションリマインダーの表記が異なっていても上書きしない、`--amend` での書き換えもしない）
- **プランに記載の無い判断（トレーラーの表記など）で迷っても作業を止めずに進め、報告に「要確認」として記載する**こと

### 2. レビュー（メインの責務）

サブエージェントの成果を必ずレビューする:

- テストが**先に**書かれ、Red→Greenを経ているか。**報告されたRed時の失敗出力を確認**し、テストと実装を同時に書く TDD 違反が無いか検証する
...
- 報告に記載された「要確認」事項を確認し、次回以降の委譲プランに反映する
- 不足・誤りがあれば具体的な指摘を返して再依頼する
- **コミット前に、主要な条件分岐を一時的に壊して（ミューテーション）テストがRedになることを確認する**
...
- サブエージェントには必ずメインエージェントより下位のモデルを使う
