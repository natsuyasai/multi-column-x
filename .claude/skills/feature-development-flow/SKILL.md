...
digraph flow {
  ...
  "5. 完了処理(format/lint/test/build)" -> "6. 結合テスト観点出力(任意)" [label="複数画面/モジュールにまたがる変更等"];
}
```
...
| 6. 結合テスト観点出力（任意） | 自動チェックでは検出できない手動QA観点を出力する。複数画面・複数モジュールにまたがる変更等で有用                    | `integration-test-viewpoints`                                                               |
