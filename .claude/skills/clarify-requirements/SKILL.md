...
- [ ] 解決したい課題・背景（なぜやるのか）
- [ ] 対象範囲（どのカラム・コンポーネント・機能か）
- [ ] 期待する振る舞い（入力・出力・エッジケース）
- [ ] 非対象（やらないこと）の明示
- [ ] 制約（既存仕様・パフォーマンス・互換性・テーマ・desktop/mobileの差異）
- [ ] 完了の判断基準（何が満たされれば完了か）
...
合意できたらフェーズ1.5（`gherkin-spec`）でGherkin仕様を定義する（機能追加・挙動変更・バグ修正の場合は必須。リファクタ・docs・CI設定のみの変更はフェーズ2へ進む）。
