...
| 表示                           | ヘッダーの非表示、投稿欄の非表示、カスタムメニューボタンの表示、写真閲覧後のスクロール位置復元、（ホームカラムのみ）更新後に前回の続きへ戻るボタンを表示 |
...
- **ポップアップウィンドウ** — Esc キーで閉じるかどうか、画像／動画をそれぞれポップアップウィンドウで開くかどうか。
- **動画** — 動画の自動再生を停止するかどうか。
- **広告** — 広告を非表示にするかどうか。
...
同じ設定セクションの **「広い画面で2カラム表示」** を有効にすると、幅の広い画面（横向き・タブレットなど）でカラムを 2 つ並べて表示できます（対応端末のみ）。
