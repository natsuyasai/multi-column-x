...
- **フロントの案内 UI は存在しない**。以前あった `useMediaCodecCheck.ts`（起動時チェック）・`CodecWarningDialog.tsx`（案内・ダウンロードボタン）・TopBar の警告アイコンは、コミット `4dabce4`（2026-08-15「コーデック欠如警告UIを削除」）で削除した。Rust 側の `check_media_codec_support` と `download_and_enable_h264` は維持しているが、現在フロントからの呼び出し口は無い。案内 UI を復活させる場合は、`4dabce4` の親コミットの実装が参考になる。
- `download_and_enable_h264` はダウンロード成功後に GStreamer のレジストリキャッシュを削除する（反映にはアプリの再起動が必要）。
...
**変更時の注意点**: 案内 UI を復活させる場合は、アプリ内に表示する手動インストールコマンド文字列と `install.sh` の `print_codec_hint()` の同コマンド文字列を完全一致させること（一方だけ更新すると案内内容がアプリ内とインストーラで食い違う）。Cisco OpenH264 のバージョン・SHA256（`openh264_fetch.rs` の定数）を更新する場合は、`apt-get download libopenh264-cisco7 && dpkg-deb -e` で実際の postinst スクリプトから最新値を取得して反映すること（憶測で埋めない）。
