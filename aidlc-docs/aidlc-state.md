...
| 依頼                           | 対応                                                                                                                                                                                                                                                      | 状態 |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 新着通知が元から動作しない     | 静的解析で確定原因1件: `Notification.requestPermission()` が一度も呼ばれず permission が常に default → 通知分岐が実行不能。WebView2 の Web Notification 不安定性も踏まえ `tauri-plugin-notification` へ置換（`fix/desktop-notification-plugin` で実装中） | 🔄   |
| 同上・inject 検知の実 DOM 検証 | `isFollowingTabActive` の aria-expanded ヒューリスティックと新着ピルセレクタ、通知ページの検知手段が要検証。**Chrome 拡張未接続のためブロック中**（ユーザーによる拡張の接続待ち）                                                                         | ⏸    |
