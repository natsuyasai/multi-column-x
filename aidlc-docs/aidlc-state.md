...
| 9        | AppSettingsPanel リファクタ（R1）                           | `refactor/app-settings-panel` | 🔄 実行中（Task 9.1 は 93b384b でコミット済み、Task 9.2 のセクション分割作業中。進行が低速のため督促済み）                                                                         |
...
| 10       | 通知対象拡大（U4）/ プリセットモバイル（U5）                | `feat/notify-any-column`      | 🔄 Task 10.1（カラム別通知）のみ 2026-07-03 起動（`fix/security-hardening` 起点。AppSettingsPanel 非依存のため前倒し）。Task 10.2（プリセットモバイル）はフェーズ9完了待ち         |
